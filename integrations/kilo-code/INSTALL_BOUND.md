...
> **Type: Instruction-only.** This integration provides a prompt that Kilo Code
> reads and follows. It is **not enforced** — there are no programmatic hooks
> into Kilo Code's event system, modes, rules, MCP servers, or lifecycle events.
> BOUND evaluates decisions; Kilo Code is responsible for acting on them.
...
task runner, your CI, or your agent loop. Kilo Code is the execution surface that wires it in.

This is an integration prompt, not a native Kilo Code plugin. BOUND ships no
Kilo-Code-specific code. Do not assume hooks, modes, rules, slash commands,
MCP servers, checkpoints, or lifecycle events. Inspect the installed version
and workspace, and use only mechanisms confirmed there.
...
> not decide what code to write.** You (Kilo Code) decide what code to write; BOUND decides
...
- Inspect whether this workspace actually provides Kilo Code rules, modes,
  commands, MCP tools, task boundaries, or checkpoints. Treat each as
