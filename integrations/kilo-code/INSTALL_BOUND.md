# Integration prompt for Kilo Code — Install BOUND

> This document is **not documentation for a human.** It is a prompt designed
> to be pasted directly into **Kilo Code**. Paste everything below the line
> into a Kilo Code session and let it run.

---

You are Kilo Code. Your job is to integrate **BOUND** — a deterministic
...
This is an **integration prompt for Kilo Code**, not a native Kilo Code plugin.
BOUND ships no Kilo-Code-specific code and no native integration. Do not
pretend a native integration exists. Do not assume any undocumented hook,
instruction file, mode, command, or workflow mechanism. Before relying on a
mechanism, **inspect what is actually available in this environment** and use
only that.
...
> not decide what code to write.** You (Kilo Code) decide what code to write;
> BOUND decides whether the step you just took is good enough to move on.

## The conceptual integration boundary

    Kilo Code executes a meaningful task / subtask
            ↓
    ...
    BOUND evaluates  (deterministic)
            ↓
    Kilo Code reacts to the decision
...
## Step 1 — Inspect what Kilo Code mechanisms are ACTUALLY available here

Do **not** assume undocumented hooks, modes, instruction files, slash commands,
or workflow mechanisms. Kilo Code's available mechanisms vary by version and
workspace configuration.

Inspect and record what is actually available in *this* environment:

- **Execution surface**: What can Kilo Code run as a "meaningful step"? (A
  task, a subtask, a delimited phase, a checkpoint between tool batches.) Which
  granularity is right to evaluate?
- **Available mechanisms**: What does this environment actually expose? Check
  for: custom instructions / config files that actually exist here, available
  modes, slash commands, any tool-call or stop hooks only if present. Use only
  what you confirm exists.
...
```text
Integration point:
    Where in the Kilo Code workflow BOUND is called.

Step boundary:
    The concrete Kilo Code tasks/subtasks you will map to StepContracts, and
...
- **ROLLBACK** → restore a safe state where possible (e.g. `git checkout` /
  restore a Kilo Code checkpoint); then replan. BOUND does not execute the
  rollback; you do.

## Rules you must not break

- **Never assume undocumented Kilo Code hooks.** Only use mechanisms you
...
- **Do not hardcode Kilo-Code-specific behavior into `src/bound/`.** BOUND
...
Summarize: the Kilo Code mechanisms you actually used (and confirmed exist),
...
Control-flow mapping:
    How each BOUND decision changes what Kilo Code does next (see Step 5).
