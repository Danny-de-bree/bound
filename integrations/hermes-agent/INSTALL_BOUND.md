...
task runner, your CI, or your agent loop. Hermes Agent is the execution surface that wires it in.

This is an integration prompt, not a native Hermes Agent plugin. BOUND ships
no Hermes-specific code. Do not assume hooks, instruction files, commands,
workflow steps, tool interceptors, or lifecycle events. Inspect the installed
environment and use only mechanisms confirmed there.
...
> not decide what code to write.** You (Hermes Agent) decide what code to write; BOUND decides
...
- Inspect whether this environment actually provides Hermes instructions,
  planning facilities, commands, delegated agents, tool hooks, or checkpoints.
  Treat each as unavailable until confirmed.
