...
You are Hermes Agent. Your job is to integrate **BOUND** — a deterministic
...
This is an **integration prompt for Hermes Agent**, not a native Hermes plugin.
BOUND ships no Hermes-specific code and no native integration. Do not pretend a
native integration exists. Do not assume any undocumented hook, instruction
file, workflow step, command, or lifecycle event. Before relying on a
...
> not decide what code to write.** You (Hermes Agent) decide what code to
> write; BOUND decides whether the step you just took is good enough to move on.

## The conceptual integration boundary

    Hermes Agent executes a meaningful task / subtask
            ↓
    ...
    BOUND evaluates  (deterministic)
            ↓
    Hermes Agent reacts to the decision
...
## Step 1 — Inspect what Hermes Agent mechanisms are ACTUALLY available here

Do **not** assume undocumented hooks, instruction files, workflow steps,
commands, or lifecycle events. Hermes Agent's available mechanisms vary by
version and configuration.

Inspect and record what is actually available in *this* environment:

- **Execution surface**: What can Hermes run as a "meaningful step"? (A
  workflow step, a subtask, a delimited phase, a checkpoint between tool
  batches.) Which granularity is right to evaluate?
- **Available mechanisms**: What does this environment actually expose? Check
  for: instruction / config files that actually exist here, available commands,
  workflow step definitions, any tool-call or stop hooks only if present. Use
  only what you confirm exists.
...
```text
Integration point:
    Where in the Hermes Agent workflow BOUND is called.

Step boundary:
    The concrete Hermes tasks/subtasks you will map to StepContracts, and why
...
- **ROLLBACK** → restore a safe state where possible (e.g. `git checkout` /
  restore a Hermes checkpoint); then replan. BOUND does not execute the
  rollback; you do.

## Rules you must not break

- **Never assume undocumented Hermes hooks.** Only use mechanisms you
...
- **Do not hardcode Hermes-specific behavior into `src/bound/`.** BOUND stays
  framework-neutral; all wiring lives in this project's files.
...
Summarize: the Hermes mechanisms you actually used (and confirmed exist), files
created/modified, `bound-policy` version installed, one real `StepContract` +
decision, and confirmation no evidence was fabricated and no BOUND policy logic
was duplicated.
...
Control-flow mapping:
    How each BOUND decision changes what Hermes does next (see Step 5).
