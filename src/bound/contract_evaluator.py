"""Deterministic contract evaluator (v0.3 Phases 7 & 8).

The :class:`ContractEvaluator` is the deterministic seam that turns a
:class:`~bound.contracts.StepContract` plus the
:class:`~bound.evidence.ExecutionEvidence` collected after an agent executed
that step into the four BOUND score dimensions — ``A``, ``I``, ``R``, ``C`` —
*without* an LLM. It sits at the end of the v0.3 contract pipeline:

.. code-block:: text

    StepContract + ExecutionEvidence → ContractEvaluator → A / I / R / C
        → BoundCalculator → BoundPolicy → ACCEPT / RETRY / REPLAN / ROLLBACK

The key v0.3 principle — *use an LLM to translate intent into an explicit
evaluation contract, not to make the final BOUND decision* — means the
contract and evidence carry all the structure; the final scores are a pure,
deterministic, fully-documented function of those two inputs. There is no
network access and no LLM SDK: once the contract and evidence are supplied the
scores are bit-for-bit reproducible.

This evaluator does **not** implement the :class:`~bound.evaluator.Evaluator`
Protocol (that seam scores a proposed :class:`~bound.models.Action`); it is the
contract-based analogue that scores an *executed* step against its declared
contract. The decision (ACCEPT / RETRY / REPLAN / ROLLBACK) remains the
exclusive responsibility of the deterministic :class:`~bound.policy.BoundPolicy`.

Provenance contract (Phase 8)
-----------------------------
:class:`~bound.models.EvaluationScores` has no provenance field, so this
evaluator exposes per-dimension evidence two complementary ways, mirroring
:class:`~bound.workflow.CodingWorkflowEvaluator`:

1. ``ContractEvaluator.provenance`` — a ``dict[str, list[ScoreEvidence]]`` keyed
   by ``"acceptance"``, ``"influence"``, ``"risk"``, ``"cost"``, populated on
   every :meth:`evaluate` call. A policy or experiment harness can read it and
   attach it to :attr:`~bound.models.EvaluationResult.provenance`.
2. ``EvaluationScores.reasoning`` — a short, structured, human-readable summary
   of every rule and the resulting values, so the scores are self-explaining
   even without the result object.

A consumer must be able to answer "why is ``A = 0.75``?" by reading the
provenance — e.g. the acceptance summary reads "✓3 of 4 required check(s)
passed: a, b, c; ✗ d. A = 3/4 = 0.7500."

Overarching honesty principle
-----------------------------
For anything *declared* on the contract (acceptance checks, risk checks, budget
dimensions), missing confirming evidence is scored **conservatively** — it is
never silently assumed to pass, be safe, or be under budget. For *pure*
execution observables that the contract does not declare (the presence of
unexpected artifacts, rollback availability), an unmeasured (``None``) value is
*skipped* rather than invented, so a clean step is not penalised for unmeasured
telemetry. Both rules are deterministic and documented per dimension below.
"""
